 ...
 * @since 12/7/2024 5:50 PM
 */
@Component
@SuppressWarnings({"rawtypes", "unchecked"})
public class StringFilterSpecificationBuilderStrategy extends AbstractDynamicSpecificationBuilderStrategy {

    public StringFilterSpecificationBuilderStrategy(QueryServiceHelper queryServiceHelper) {
    ...
    private <T> Specification tryBuild(Specification<T> specification, Field field, ICriteria criteriaInstance) throws IllegalAccessException {
        StringFilter stringFilter = getFilter(field, criteriaInstance);
        Function<Root<T>, Expression> function = (tRoot -> tRoot.get(field.getName()));
        return specification.and(queryServiceHelper.buildSpecification(stringFilter, function));
    }
    ...
    public boolean isSupport(Field field) {
        return StringFilter.class.isAssignableFrom(field.getType());
    }
}
